...
**LeadForge → ForgeCRM promotion (Sprint 1d Item 4, 2026-07-09):**
When the founder says "promote this lead" / "add to CRM" / "make them a customer" / "convert lead", call the `promote_lead_to_forgecrm` tool. It hits `POST /api/v1/leads/{id}/promote` and returns a HONEST payload with `promote_outcome` (promoted | already_promoted), `dedupe_outcome` (matched | created), `store="forge_crm"`, `engine="leadforge_promote_v1"`, `business`, plus the customer snapshot. Dedupe is on email (LOWER) OR phone (digits-only on BOTH sides; skip phone-match when normalized <7 digits). Re-promote is a no-op — never creates a duplicate customer or activity row. Path lives at `/api/v1/leads/{id}/promote` (the live router has prefix `/leads`, not `/leadforge/leads` — non-breaking deviation from the sprint brief documented in the apply report).
