...
def _email_status() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    email_channel = _channel(
        ...
        last_error_category=None,
        evidence=[
            "Email operational in guarded/manual founder mode — not a full auto-reply assistant loop.",
            ...
            "Auto-reply remains disabled; every live reply requires explicit founder approval.",
        ],
        next_required_action="Keep auto-reply disabled. Enable live MAX email reply loop only after founder explicitly approves auto-reply.",
...
def _hermes_channel_status() -> dict[str, Any]:
    ...
    layers = [
        ...
        _layer(
            ...
            STATUS_PARTIAL,
            evidence=[
                f"Hermes dashboard port 9119 reachable: {hermes['external_hermes_reachable']}",
                f"Hermes gateway running: {hermes.get('external_hermes_gateway_running')} — dashboard accessible but gateway process is not active.",
                f"Hermes version: {(hermes.get('external_hermes_api') or {}).get('version') or 'unknown'}",
            ],
            next_action="Hermes dashboard is reachable but gateway is not running. Gateway must be started before external channels can function.",
            details={
                "port": 9119,
                "dashboard_reachable": hermes["external_hermes_reachable"],
    ...
    return _channel(
        ...
        evidence=[
            ...
            f"Hermes dashboard port 9119 reachable: {hermes['external_hermes_reachable']}",
            f"Hermes gateway running: {hermes.get('external_hermes_gateway_running')} (dashboard accessible but gateway process not active)",
            f"External Hermes Telegram: unverified (no live status endpoint available)",
            f"Hermes Telegram connected: {telegram_connected}",
            f"Hermes cron: paused",
        ...
        next_required_action="Hermes gateway must be running before external Hermes channels can be verified. Keep cron paused.",
        safe_to_live_test=False,
        live_test_required=False,
        layers=layers,
    )
