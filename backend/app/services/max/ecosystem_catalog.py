...
EMPIRE_CATALOG = {
    ...
    "tools": {
        ...
        "create_quick_quote": {"description": "DEPRECATED — legacy JSON store (store=json_legacy, engine=qis); use create_engine_quote for new quotes", "access_level": 1, "category": "action"},
        "create_engine_quote": {"description": "Create canonical quote in quotes_v2 via pricing_engine_v1 (store=quotes_v2, engine=pricing_engine_v1). Multi-line via line_items[]. Returns per-line proposed_price + computed_json.", "access_level": 1, "category": "action"},
        "create_invoice_from_quote": {"description": "Sprint 1d Payment Phase 1: explicit quote→invoice action. Gated to quote.status in {sent, accepted, in_production, completed}; returns 409 otherwise. Returns invoice snapshot from canonical invoices table (store=quotes_v2, engine=lifecycle_v1).", "access_level": 1, "category": "action"},
        "promote_lead_to_forgecrm": {"description": "Sprint 1d Item 4: promote a LeadForge lead to a ForgeCRM customer (store=forge_crm, engine=leadforge_promote_v1). Dedupe on email (LOWER) or phone (digits-only on both sides; skip phone-match when normalized <7 digits). Re-promote is a no-op (already_promoted). Business-scoped.", "access_level": 1, "category": "action"},
        "submit_quote_for_review": {"description": "Sprint 1c: move a draft quote into the founder_review queue (agents can call; access_level=1).", "access_level": 1, "category": "action"},
        "list_quotes_awaiting_review": {"description": "Sprint 1c: list founder_review quotes + legacy 'proposal' quotes (tagged pending_migration=True).", "access_level": 1, "category": "action"},
        "show_quote_for_review": {"description": "Sprint 1c: show a quote's line items with proposed vs final + delta. Annotated with state_metadata.legacy/pending_migration.", "access_level": 1, "category": "action"},
        "approve_quote": {"description": "Sprint 1c: founder-only transition founder_review → sent (FOUNDER ONLY, access_level=0). Does NOT auto-send the PDF/email.", "access_level": 0, "category": "action"},
        "reject_quote": {"description": "Sprint 1c: founder-only transition founder_review → draft (FOUNDER ONLY, access_level=0).", "access_level": 0, "category": "action"},
