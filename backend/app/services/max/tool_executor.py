...
def _send_quote_telegram(params: dict, desk: Optional[str] = None) -> ToolResult:
    ...
    try:
        ...
        sent = bool(_run_async(telegram_bot.send_document(pdf_path, caption=caption)))
        if not sent:
            return ToolResult(tool="send_quote_telegram", success=False, error="Telegram document send was not verified")
    except Exception as tg_err:
        logger.warning(f"Telegram document send failed: {tg_err}")
        return ToolResult(tool="send_quote_telegram", success=False, error=f"Telegram document send failed: {tg_err}")
...
<body>{svg_content}</body></html>"""
        ...
        if size <= 0:
            return ToolResult(tool="svg_to_pdf", success=False, error="PDF generation produced an empty file")
...
<p style="color:#888;font-size:12px">Generated by MAX — Empire AI</p>"""
        sent = svc.send(to=email_to, subject=subject, body_html=body, attachments=[pdf_path])
        if not sent:
            return {"emailed": False, "error": "Email provider did not verify send acceptance"}
...
def _present(params: dict, desk: Optional[str] = None) -> ToolResult:
    ...
    if not pdf_path or not os.path.exists(pdf_path):
        return ToolResult(tool="present", success=False, error="PDF rendering verification failed: file missing")
    pdf_size = os.path.getsize(pdf_path)
    if pdf_size <= 0:
        return ToolResult(tool="present", success=False, error="PDF rendering verification failed: empty file")
...
### Presentation Tools
- **present** — Generate a professional presentation/report on a topic and verify a local PDF artifact. It does not send externally.
  ...
  To deliver a generated PDF externally, use the specific email or Telegram delivery tool and only report delivery after that tool succeeds.
