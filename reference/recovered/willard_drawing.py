...
# dimension: back arc 87"
c.setFont("Helvetica-Bold", 9); c.setFillColor(DIM)
c.drawCentredString(cx, cy + Rb * S + 10, '87" ALONG BACK SIDE OF CURVE')
# depth dimension (radial, offset right of center to clear fill)
d1 = pt(Rf, 0.12); d2 = pt(Rb, 0.12)
dim_v(d1[0] + 6, d1[1], d2[1] - 2, '26" SEAT DEPTH', right=True)
# radius note
...
Se = 3.2
ex, ey = 60, 80                       # bottom-left of elevation
...
c.setFillColor(CARAMEL); c.setLineWidth(1.4)
c.roundRect(sx, sy + leg_h, sw, body_h, 4, stroke=1, fill=1)
dim_h(sx, sx + sw, sy - 14, '26"', above=False)
dim_v(sx - 14, sy, sy + leg_h + body_h, '17"', right=False)
view_label(sx + sw / 2, sy + leg_h + body_h + 16, "SIDE ELEVATION")

# ---- ISOMETRIC ASSEMBLY VIEW (center-right) -----------------------------
iso_ox, iso_oy, iso_s = 300, 220, 2.0
def iso(x, y, z):
    return (iso_ox + (x * 0.95 + y * 0.45) * iso_s,
            iso_oy + (z * 1.0 + y * 0.28) * iso_s)
def face(pts, fill, lw=0.9):
    p = c.beginPath()
    p.moveTo(*iso(*pts[0]))
    for q in pts[1:]:
        p.lineTo(*iso(*q))
...
# panel (behind): front face y=27, thickness 2, z 17..41
face([(0, 29, 17), (87, 29, 17), (87, 29, 41), (0, 29, 41)], CAR_SIDE, 0.6)
face([(0, 27, 41), (87, 27, 41), (87, 29, 41), (0, 29, 41)], CAR_TOP)
face([(87, 27, 17), (87, 29, 17), (87, 29, 41), (87, 27, 41)], CAR_SIDE)
face([(0, 27, 17), (87, 27, 17), (87, 27, 41), (0, 27, 41)], CAR_FRONT)
# channel seams on panel front (7 seams)
...
# legs (front row of 3 visible)
for lx in (2, 42.25, 82.5):
    face([(lx, 2, 0), (lx + 2.5, 2, 0), (lx + 2.5, 2, 6), (lx, 2, 6)], WOOD, 0.6)
    face([(lx + 2.5, 2, 0), (lx + 2.5, 4.5, 0), (lx + 2.5, 4.5, 6), (lx + 2.5, 2, 6)], HexColor("#6f4f28"), 0.6)
# bench body z 6..17
face([(0, 0, 17), (87, 0, 17), (87, 26, 17), (0, 26, 17)], CAR_TOP)
face([(87, 0, 6), (87, 26, 6), (87, 26, 17), (87, 0, 17)], CAR_SIDE)
face([(0, 0, 6), (87, 0, 6), (87, 0, 17), (0, 0, 17)], CAR_FRONT)
# labels
c.setFont("Helvetica", 7); c.setFillColor(DIM)
la = iso(90, 0, 11); c.drawString(la[0] + 3, la[1], '17" BENCH')
la = iso(90, 27, 30); c.drawString(la[0] + 3, la[1], '24" PANEL')
la = iso(44, 0, -3); c.drawCentredString(la[0], la[1] - 6, '87"')
view_label(iso_ox + 105, iso_oy - 24, "ISOMETRIC — ASSEMBLY")
c.setFont("Helvetica-Oblique", 6.5); c.setFillColor(GRAY)
c.drawCentredString(iso_ox + 105, iso_oy - 34, "(plan curve not shown — see plan view)")
...
title_block(
    ...
    [
        ...
        'No back, no loose cushions. Fully upholstered except exposed legs.',
    ],
)
c.showPage()
