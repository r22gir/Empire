...
G=dict(W=126.0,T=.72,BAY_L=23.0,BAY_R=103.0,MOD=23.0,TOWER_D=8.0,BASE_D=17.97,
 ...
 TOP_CLR=22.5625,MID_CLR=12.25,NSH=5,UP=12.25,NIN=3,BAYS=4,
...
def elevation(ax,ay,k):
    ...
    o.append(r_(X(G["BAY_L"]+G["T"]),Y(G["H"]),G["TB"]*k,G["T"]*k,WAL,"#5A3A28",.8))
    o.append(r_(X(G["BAY_L"]+G["T"]),Y(G["ohBot"]+G["T"]),G["TB"]*k,G["T"]*k,WAL,"#5A3A28",.8))
    for ex in (G["BAY_L"],G["BAY_R"]-G["T"]):
        o.append(r_(X(ex),Y(G["H"]),G["T"]*k,G["ohBox"]*k,WAL,"#5A3A28",.9))
    p=G["BAY_L"]+G["T"]
    for i in range(G["BAYS"]):
        o.append(line(X(p+.4),Y(G["H"]-G["T"]-.5),X(p+G["bay"]-.4),Y(G["H"]-G["T"]-.5),"#E8C877",1.6))
        p+=G["bay"]
        if i<G["NIN"]:
            o.append(r_(X(p),Y(G["ohBot"]+G["T"]+G["UP"]),G["T"]*k,G["UP"]*k,WAL,"#5A3A28",.8))
            p+=G["T"]
