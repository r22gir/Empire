#!/bin/bash
# CoPilotForge v2 — One-Click End Session
...
echo ""
echo -e "${BLUE}CoPilotForge — Ending Session${NC}"
echo -e "${BLUE}═══════════════════════════════════════${NC}"
...
if [ -d "$SESSION_DIR" ] && command -v xclip &>/dev/null; then
    ...
    if [ -n "$CLIP" ] && [ ${#CLIP} -gt 50 ]; then
        ...
        {
            echo "# CoPilotForge Session — Final Save"
...
{
    echo "# CoPilotForge Session Summary"
    echo ""
    echo "| Field | Value |"
    echo "|-------|-------|"
...
cp "$SUMMARY_FILE" ~/Empire/docs/CHAT_ARCHIVE/latest_copilot.md
